...
namespace BFF
{
  ...
  {
    private BFF.BFF.InputInfo _files;
    ...
    [DllImport("XComp.dll")]
    public static extern int My_XMemCreateDecompressionContext(
      int codec_type,
      IntPtr codec_params,
      ushort flags,
      out IntPtr pContext);
    ...
    [DllImport("XComp.dll")]
    public static extern int My_XMemDecompress(
      IntPtr pContext,
      [MarshalAs(UnmanagedType.LPArray)] byte[] pDestBuffer,
      out int pDestSize,
      [MarshalAs(UnmanagedType.LPArray)] byte[] source,
    ...
    public void SetParams(string f1, string f2, string o) => this._files = new BFF.BFF.InputInfo(f1, f2, o);
    ...
    private void doInject(List<FileInfo> files, bool alwaysUpdate)
    {
      ...
      BFF.BFF.My_XMemCreateCompressionContext(1, IntPtr.Zero, (ushort) 0, out pContext);
      int num1 = 304;
      byte[] buffer1 = new byte[4];
          ...
          if (!this.StopProcessing.WaitOne(1))
          {
            ...
            if (alwaysUpdate || lastWriteTime2.CompareTo(lastWriteTime1) >= 0)
            {
              ...
              if (num3 != -1)
              {
                ...
                switch (BFF.BFF.My_XMemCompress(pContext, numArray2, out pDestSize, numArray1, length))
                {
                  case -2116149247:
                  case 0:
                    if (pDestSize > length)
    ...
    public void Build()
    {
      ...
      List<BFF.BFF.StaticData> staticData1 = this.getStaticData(fileList);
      ...
      {
        BFF.BFF.StaticData staticData2 = staticData1[index];
    ...
    private uint _getCRC(byte[] b, int size)
    {
      ...
      for (int index = 0; index < size; ++index)
        num = this._crctable[(IntPtr) (uint) (((int) b[index] ^ (int) num) & (int) byte.MaxValue)] ^ num >> 8;
      return num;
    }
    ...
    public byte[] XCompress(byte[] src, int srcLen, int usize, out int size)
    {
      ...
      if (BFF.BFF.My_XMemCompress(pContext, pDestBuffer, out pDestSize, src, srcLen) != 0)
    ...
    {
      IntPtr pContext = IntPtr.Zero;
      BFF.BFF.My_XMemCreateDecompressionContext(1, IntPtr.Zero, (ushort) 0, out pContext);
      int pDestSize = usize;
      byte[] pDestBuffer = new byte[pDestSize];
      if (BFF.BFF.My_XMemDecompress(pContext, pDestBuffer, out pDestSize, src, srcLen) != 0)
        throw new Exception("Can't decompress. Aborting processing.");
      BFF.BFF.My_XMemDestroyDecompressionContext(pContext);
      return pDestBuffer;
    }
